...
window.onclick = function(event) {
  ...
  var alertDivisions = document.getElementsByClassName("alert");
  var i;
  
  for (i = 0; i < alertDivisions.length; i++) {
	  $(alertDivisions[i]).hide();
  }
}

//Start showing processing image
...
//Reset form fields, remove valid-value-indicator class(Which displays green box around fields) and invalid-value-indicator class(Which shows red box around fields)
//Also remove error messages which we show beneath field inputs.
function resetForm(formName) {
	$("#"+formName+" label").removeClass("state-success");
	...
	$("#"+formName+" label").each(function () {
		if($(this).hasClass('state-error')) {
			...
			$(this).find('input, select, textarea').each(function() {
				var errorMessageElement = $(this).attr("aria-describedby");
